...
//-----------------------------------------------------------------------------
	...
	float OutputZoneSecs(const ProfileZone *a_root, uint32_t a_count, const char* name) {
		const ProfileZone *zone = a_root;

		const char* q;
		while (zone && ((q = strstr(zone->name, name)) == 0) )
		{
			zone = zone->next;
		}
		...
		return (float)zone->data.totalTicksAvg() * GetTickInvFreq();
	}
