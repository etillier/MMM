...
void DistanceMatrix::create_from_alignment(Alignment alignment, bool const& quick)
...
{	init();
	if (!inFile.is_open())
	{	throw runtime_error("invalid filehandle");
	}
	
	inFile >> nOfSequences_;
	if (inFile.peek() != '\n')
		throw runtime_error("invalid distance matrix format");
...
{	init();
	...
	{	//cout << "\nError: cannot open file " << inFilepath << endl;
		//exit(1);
		throw runtime_error("cannot open file: " + inFilepath);
	}
